...
class UpdateOrderToDeliveredTest(TestCase):
    def setUp(self):
    ...
    def test_update_order_to_delivered_success(self):
        ...
        response = self.client.put(reverse('order-delivered', kwargs={'pk': self.order.id}))
        ...
        self.assertIsInstance(self.order.delivered_at, timezone.datetime)

    def test_update_order_to_delivered_non_admin(self):
    ...
    def test_update_nonexistent_order(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.put(reverse('order-delivered', kwargs={'pk': 999}))
        print(colorama.Fore.MAGENTA + "Response Data:", response.json())
